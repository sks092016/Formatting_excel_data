...
def merged_line_geometry(lines, tol):
    merged_coords = []
    for line in lines:
        ...
        if not merged_coords:
            ...
            # Replace first coord of new line if it's very close to last coord
            if Point(coords[0]).distance(Point(merged_coords[-1])) <= tol:
                coords[0] = merged_coords[-1]
...
for s in span_list:
    ...
    while not remaining.empty:
        ...
        for idx, row in remaining.iterrows():
            if coords_match(row['start'], current['end'], tolerence):
            ...
            elif coords_match(row['end'], current['end'], tolerence):
            ...
            # Case 3: Reversed row connects backward
            elif coords_match(row['end'], current['start'], tolerence):
                reversed_geom = LineString(list(row['geometry'].coords)[::-1])
                temp_df.at[idx, 'geometry'] = reversed_geom
                row['geometry'] = reversed_geom
                row['start'], row['end'] = get_start_end_coords(reversed_geom)
    ...
    # Merge geometry
    complete_span_line = merged_line_geometry(temp_df.loc[sorted_indices].geometry, tolerence)
