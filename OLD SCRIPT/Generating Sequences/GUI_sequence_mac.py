"""
OFC Network Sequencer - Fixed GUI Application
Now works on Mac and supports editing existing rings
"""
...
class OFCSequencerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("OFC Network Sequencer v2.0 (Mac Compatible)")
        ...
        # Ring coordinates - now stored as list of dicts for easier editing
        self.rings = [
            {'name': 'R1', 'coordinate': ''},
            {'name': 'R2', 'coordinate': ''},
            {'name': 'R3', 'coordinate': ''},
...
"""
    ...
    def create_widgets(self):
        ...
        # Tab 2: Ring Coordinates (NEW - with edit/delete support)
        ...
        notebook.add(process_frame, text='Process & Logs')
    ...
    def create_rings_tab(self, parent):
        """Create rings configuration tab with edit/delete support."""
        info = ttk.Label(parent,
                         text="Configure start coordinates for each ring. Leave blank to use BHQ coordinate.\nYou can edit existing rings or add new ones.",
        ...
        ttk.Button(btn_frame, text="+ Add New Ring",
                   command=self.add_new_ring).pack(side='left', padx=5)

    def refresh_rings_display(self):
        """Refresh the rings display."""
        # Clear existing widgets
        for widget in self.rings_container.winfo_children():
            widget.destroy()

        # Header
        header_frame = ttk.Frame(self.rings_container)
        header_frame.pack(fill='x', pady=(0, 5))

        ttk.Label(header_frame, text="Ring Name", width=15,
                  font=('Arial', 10, 'bold')).pack(side='left', padx=5)
        ttk.Label(header_frame, text="Coordinate (lon lat)", width=40,
                  font=('Arial', 10, 'bold')).pack(side='left', padx=5)
        ttk.Label(header_frame, text="Actions", width=15,
                  font=('Arial', 10, 'bold')).pack(side='left', padx=5)
        ...
        for idx, ring in enumerate(self.rings):
            ring_frame = ttk.Frame(self.rings_container)
            ring_frame.pack(fill='x', pady=2)

            # Ring name entry
            name_entry = ttk.Entry(ring_frame, width=15)
            name_entry.insert(0, ring['name'])
            name_entry.pack(side='left', padx=5)

            # Coordinate entry
            coord_entry = ttk.Entry(ring_frame, width=40)
    ...
    def update_ring(self, index):
        ...
        if new_name:
            ...
            messagebox.showinfo("Success", f"Ring '{new_name}' updated successfully!")
        else:
    ...
    def delete_ring(self, index):
        ...
        if messagebox.askyesno("Confirm Delete",
                               f"Are you sure you want to delete ring '{ring['name']}'?"):
            self.rings.pop(index)
            self.refresh_rings_display()
            messagebox.showinfo("Success", "Ring deleted successfully!")
    ...
    def add_new_ring(self):
        ...
        ttk.Label(dialog, text="(Format: 'lon lat' or leave blank for BHQ)",
                  font=('Arial', 8, 'italic')).grid(row=2, column=1, sticky='w', padx=10)
        ...
        def add_ring():
            ...
            if name:
                ...
                self.rings.append({
                    'name': name,
                    'coordinate': coord.get().strip()
                })
                self.refresh_rings_display()
                messagebox.showinfo("Success", f"Ring '{name}' added successfully!")
                dialog.destroy()
            else:
                messagebox.showerror("Error", "Ring name cannot be empty!")

        ttk.Button(dialog, text="Add Ring", command=add_ring).grid(row=3, column=1, pady=15, sticky='e')
        ttk.Button(dialog, text="Cancel", command=dialog.destroy).grid(row=3, column=0, pady=15, sticky='e')
...
T-POINT CHHATERA: 79.81278780, 21.57607810"""
    ...
    def create_process_tab(self, parent):
        ...
        ttk.Button(btn_frame, text="1. Check GP Names",
                   command=self.check_gp_names, width=20).pack(side='left', padx=5)
        ttk.Button(btn_frame, text="2. Generate Sequences",
        ...
        # Use appropriate colors based on platform
    ...
    def save_config(self):
        ...
        if filename:
            # Update ring values from widgets before saving
    ...
    def generate_sequences(self):
        ...
        def run_generation():
            try:
                ...
                # Update ring values from widgets before processing
                for ring in self.rings:
                    if 'name_widget' in ring and ring['name_widget'].winfo_exists():
                        ring['name'] = ring['name_widget'].get().strip()
                        ring['coordinate'] = ring['coord_widget'].get().strip()
                ...
                for ring in self.rings:
                    coord = ring['coordinate'].strip()
                    rings[ring['name']] = coord if coord else bhq
...
def main():
    ...
    if platform.system() == 'Darwin':  # macOS
        ...
        try:
            # Try to set appearance for macOS
    ...
    # Choose appropriate theme based on platform
